                        ...
                        <table class="table table-bordered">
                            ...
                            @foreach($records as $record)
                                <tr id="removable{{$record->id}}">
                                    ...
                                    <td>{{$record->phone}}</td>
                                    <td class="text-center" style="direction: ltr">{{optional($record->bloodType)->name}}</td>
                                    <td>{{$record->donation_last_date}}</td>
                                    <td>{{optional($record->city)->name}}</td>
                                    <td class="text-center">
                                        @if($record->is_active)
                                            <a href="{{url(route('dashboard.clients.toggle-activation',$record->id))}}" class="btn btn-danger btn-xs">إلغاء
                                                التفعيل</a>
                                        @else
                                            <a href="{{url(route('dashboard.clients.toggle-activation',$record->id))}}"
                                               class="btn btn-success btn-xs">تفعيل</a>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        <button id="{{$record->id}}" data-token="{{ csrf_token() }}"
                                                data-route="{{URL::route('dashboard.clients.destroy',$record->id)}}"
